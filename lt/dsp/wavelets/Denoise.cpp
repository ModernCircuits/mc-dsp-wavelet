                ...
                char const* thresh, char const* level, float* denoised) -> void
{
    ...
    if (method == string_view{"dwt"})
    {
        wt.extension(ext == string_view{"per"} ? SignalExtension::periodic : SignalExtension::symmetric);
        dwt(wt, signal);
    }
    else if (method == string_view{"swt"}) { swt(wt, signal); }
    else
    {
    ...
    for (it = 0; cmp_less(it, j); ++it)
    {
        ...
        if (thresh == string_view{"hard"})
        {
            for (std::size_t i = 0; i < dlen; ++i)
            {
                if (fabs(wt.output()[iter + i]) < td) { wt.output()[iter + i] = 0; }
            }
        }
        else if (thresh == string_view{"soft"})
        {
            for (std::size_t i = 0; i < dlen; ++i)
            {
                if (fabs(wt.output()[iter + i]) < td) { wt.output()[iter + i] = 0; }
    ...
    if (method == string_view{"dwt"}) { idwt(wt, denoised); }
    else if (method == string_view{"swt"}) { iswt(wt, denoised); }
}
    ...
    for (it = 0; cmp_less(it, j); ++it)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                for (auto i = 0; i < dwtLen; ++i) { dout[i] = std::fabs(wt.output()[len + i] / sigma); }
                ...
                for (auto i = 0; i < dwtLen; ++i)
                {
                    risk[i] = ((float)dwtLen - 2 * ((float)i + 1) + dsum[i] + dout[i] * ((float)dwtLen - 1 - (float)i))
                              / (float)dwtLen;
                }
        ...
        if (thresh == string_view{"hard"})
        {
            for (auto i = 0; i < dwtLen; ++i)
            {
                if (fabs(wt.output()[len + i]) < td) { wt.output()[len + i] = 0; }
            }
        }
        else if (thresh == string_view{"soft"})
        {
...
auto denoise(DenoiseSet& obj, float* signal, float* denoised) -> void
{
    ...
    {
        if (obj.wmethod == string_view{"MODWT"})
        {
            fmt::printf("sureshrink method only works with swt and dwt. Please use setDenoiseWTMethod to set the "
                        "correct method\n");
            std::exit(-1);
        }
        sureshrink(signal, obj.N, obj.J, obj.wname.c_str(), obj.wmethod.c_str(), obj.ext.c_str(), obj.thresh.c_str(),
                   obj.level.c_str(), denoised);
    }
    else if (obj.dmethod == "visushrink")
    {
        if (obj.wmethod == string_view{"MODWT"})
        {
            fmt::printf("visushrink method only works with swt and dwt. Please use setDenoiseWTMethod to set the "
                        "correct method\n");
            std::exit(-1);
        }
        visushrink(signal, obj.N, obj.J, obj.wname.c_str(), obj.wmethod.c_str(), obj.ext.c_str(), obj.thresh.c_str(),
                   obj.level.c_str(), denoised);
    }
    else if (obj.dmethod == "modwtshrink")
    {
        ...
        {
            fmt::printf(
                "modwtshrink method only works with modwt. Please use setDenoiseWTMethod to set the correct method\n");
            std::exit(-1);
        }
        modwtshrink(signal, obj.N, obj.J, obj.wname.c_str(), obj.cmethod.c_str(), obj.ext.c_str(), obj.thresh.c_str(),
                    denoised);
    }
    else
    {
...
{
    if (strcmp(dmethod, "sureshrink") == 0) { obj.dmethod = "sureshrink"; }
    else if (strcmp(dmethod, "visushrink") == 0) { obj.dmethod = "visushrink"; }
    else if (strcmp(dmethod, "modwtshrink") == 0) { obj.dmethod = "modwtshrink"; }
    else
    {
...
{
    obj.wmethod = wmethod;
    if (!((wmethod == string_view{"dwt"}) || (wmethod == string_view{"swt"}) || (wmethod == string_view{"MODWT"})))
    {
...
{
    if (strcmp(extension, "sym") == 0) { obj.ext = "sym"; }
    else if (strcmp(extension, "per") == 0) { obj.ext = "per"; }
    else
    {
        fmt::printf("Signal extension can be either per or sym");
        std::exit(-1);
...
auto setDenoiseParameters(DenoiseSet& obj, char const* thresh, char const* level) -> void
{
    ...
    // Set thresholding
    if (thresh == string_view{"soft"}) { obj.thresh = "soft"; }
    else if (thresh == string_view{"hard"}) { obj.thresh = "hard"; }
    else
    {
    ...
    if (level == string_view{"first"}) { obj.level = "first"; }
    else if (level == string_view{"all"}) { obj.level = "all"; }
    else
    {
...
auto median(float* const x, int n) -> float
{
    ...
    float sigma = NAN;
    if ((n % 2) == 0) { sigma = (x[n / 2 - 1] + x[n / 2]) / 2.0F; }
    else { sigma = x[n / 2]; }
