...
namespace
{
auto pow2Ceil(std::size_t x) { return static_cast<std::size_t>(std::pow(2, std::ceil(std::log2(x)))); }
}  // namespace
...
FloatSignal::FloatSignal(float* data, size_t size) : FloatSignal(size) { std::copy(data, data + size, data_); }

FloatSignal::FloatSignal(float* data, size_t size, size_t padBef, size_t padAft) : FloatSignal(size + padBef + padAft)
{
...
FftForwardPlan::FftForwardPlan(FloatSignal& fs, ComplexSignal& cs)
    : FftPlan(fftwf_plan_dft_r2c_1d(static_cast<int>(fs.size()), fs.data(), reinterpret_cast<fftwf_complex*>(cs.data()),
                                    FFTW_ESTIMATE))
{
...
FftBackwardPlan::FftBackwardPlan(ComplexSignal& cs, FloatSignal& fs)
    : FftPlan(fftwf_plan_dft_c2r_1d(static_cast<int>(fs.size()), reinterpret_cast<fftwf_complex*>(cs.data()), fs.data(),
                                    FFTW_ESTIMATE))
{
    LT_ASSERT(cs.size() == (fs.size() / 2U + 1U));
}
