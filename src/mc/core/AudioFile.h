...
public:
    ...
    /** Sets the sample rate for the audio file. If you use the save() function, this sample
     * rate will be used */
...
private:
    ...
    auto writeDataToFile(std::vector<uint8_t>& fileData, std::string const& filePath)
        -> bool;
...
auto AudioFile<T>::decodeWaveFile(std::vector<uint8_t>& fileData) -> bool
{
    ...
    if (audioFormat != WavAudioFormat::PCM && audioFormat != WavAudioFormat::IEEEFloat
        ...
        reportError("ERROR: this .WAV file is encoded in a format that this library does "
    ...
    for (int i = 0; i < numSamples; i++) {
        for (int channel = 0; channel < numChannels; channel++) {
            ...
            if ((sampleIndex + (bitDepth_ / 8) - 1) >= fileData.size()) {
                reportError("ERROR: read file error as the metadata indicates more samples "
            ...
            if (bitDepth_ == 8) {
                ...
                if ((sampleAsInt & 0x800000) != 0) {  // if the 24th bit is set, this is a
        ...
        for (int channel = 0; channel < numChannels; channel++) {
            ...
            if (bitDepth_ == 8) {
                ...
                sampleAsInt         = (fileData[sampleIndex] << 16)
                            | (fileData[sampleIndex + 1] << 8) | fileData[sampleIndex + 2];
...
auto AudioFile<T>::saveToWaveFile(std::string filePath) -> bool
{
    ...
    // The file size in bytes is the header chunk size (4, not counting RIFF and WAVE) + the
    // format chunk size (24) + the metadata part of the data chunk plus the actual data
    // chunk size
...
auto AudioFile<T>::saveToAiffFile(std::string filePath) -> bool
{
    ...
    // The file size in bytes is the header chunk size (4, not counting FORM and AIFF) + the
    // COMM chunk size (26) + the metadata part of the SSND chunk plus the actual data chunk
    // size
...
auto AudioFile<T>::getIndexOfString(
    std::vector<uint8_t>& source,
    std::string const& stringToSearchFor
) -> int
{
    int index        = -1;
