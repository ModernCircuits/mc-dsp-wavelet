...
auto idwtShift(int shift, int rows, int cols, float const* lpr, float const* hpr, int lf, float* a, float* h, float* v,
               float* d, float* oup) -> void
{
    ...
    for (auto i = 0; i < ic; ++i)
    {
        ...
        for (auto k = lf / 2 - 1; k < 2 * ir + lf / 2 - 1; ++k) { cL[(k - lf / 2 + 1) * ic + i] = xLp[k]; }

        idwtPerStride(v + i, ir, d + i, lpr, hpr, lf, xLp.get(), istride, ostride);

        for (auto k = lf / 2 - 1; k < 2 * ir + lf / 2 - 1; ++k) { cH[(k - lf / 2 + 1) * ic + i] = xLp[k]; }
    }
    ...
    for (auto i = 0; i < ir; ++i)
    {
        ...
        for (auto k = lf / 2 - 1; k < 2 * ic + lf / 2 - 1; ++k) { oup[(k - lf / 2 + 1) + i * ic * 2] = xLp[k]; }
    }
...
auto imodwtPerStride(int m, float const* cA, int lenCA, float const* cD, float const* filt, int lf, float* x,
...
WaveletTransform2D::WaveletTransform2D(Wavelet& wave, char const* method, std::size_t rows, std::size_t cols,
                                       std::size_t j)
{
    ...
    this->outlength = 0;
    if (method == nullptr) { this->ext = "per"; }
    else if ((method == string_view{"dwt"}) || (method == string_view{"DWT"})) { this->ext = "per"; }
...
auto setDWT2Extension(WaveletTransform2D& wt, char const* extension) -> void
{
    ...
    else if ((wt.method() == string_view{"swt"}) || (wt.method() == "modwt"))
    {
        if (extension == string_view{"per"}) { wt.ext = "per"; }
        else
        {
...
auto dwt(WaveletTransform2D& wt, float* inp) -> std::unique_ptr<float[]>
{
    ...
    if (wt.ext == string_view{"per"})
    {
        ...
        for (iter = 0; iter < j; ++iter)
        {
            ...
            for (auto i = 0; i < ir; ++i)
            {
                dwtPerStride(orig + i * ic, ic, wt.wave().lpd().data(), wt.wave().hpd().data(), lp,
                             lpDn1.get() + i * colsI, colsI, hpDn1.get() + i * colsI, istride, ostride);
            }
            ...
            for (auto i = 0; i < ic; ++i)
            {
                dwtPerStride(lpDn1.get() + i, ir, wt.wave().lpd().data(), wt.wave().hpd().data(), lp,
                             wavecoeff.get() + aLL + i, rowsI, wavecoeff.get() + aLH + i, istride, ostride);
            }

            for (auto i = 0; i < ic; ++i)
            {
                dwtPerStride(hpDn1.get() + i, ir, wt.wave().lpd().data(), wt.wave().hpd().data(), lp,
                             wavecoeff.get() + aHL + i, rowsI, wavecoeff.get() + aHH + i, istride, ostride);
            }
    ...
    MC_ASSERT(wt.ext == string_view{"sym"});
    ...
    for (iter = 0; iter < j; ++iter)
    {
        ...
        for (auto i = 0; i < ir; ++i)
        {
            dwtSymStride(orig + i * ic, ic, wt.wave().lpd().data(), wt.wave().hpd().data(), lp, lpDn1.get() + i * colsI,
                         colsI, hpDn1.get() + i * colsI, istride, ostride);
        }
        ...
        for (auto i = 0; i < ic; ++i)
        {
            dwtSymStride(lpDn1.get() + i, ir, wt.wave().lpd().data(), wt.wave().hpd().data(), lp,
                         wavecoeff.get() + aLL + i, rowsI, wavecoeff.get() + aLH + i, istride, ostride);
        }

        for (auto i = 0; i < ic; ++i)
        {
            dwtSymStride(hpDn1.get() + i, ir, wt.wave().lpd().data(), wt.wave().hpd().data(), lp,
                         wavecoeff.get() + aHL + i, rowsI, wavecoeff.get() + aHH + i, istride, ostride);
        }
...
auto idwt(WaveletTransform2D& wt, float* wavecoeff, float* oup) -> void
{
    ...
    if (wt.ext == string_view{"per"})
    {
        ...
        for (iter = 0; iter < j; ++iter)
        {
            ...
            for (auto i = 0; i < ic; ++i)
            {
                idwtPerStride(orig + i, ir, wavecoeff + aLH + i, wt.wave().lpr().data(), wt.wave().hpr().data(), lf,
                              xLp.get(), istride, ostride);

                for (k = lf / 2 - 1; cmp_less(k, 2 * ir + lf / 2 - 1); ++k) { cL[(k - lf / 2 + 1) * ic + i] = xLp[k]; }

                idwtPerStride(wavecoeff + aHL + i, ir, wavecoeff + aHH + i, wt.wave().lpr().data(),
                              wt.wave().hpr().data(), lf, xLp.get(), istride, ostride);

                for (k = lf / 2 - 1; cmp_less(k, 2 * ir + lf / 2 - 1); ++k) { cH[(k - lf / 2 + 1) * ic + i] = xLp[k]; }
            }
            ...
            for (auto i = 0; i < ir; ++i)
            {
                idwtPerStride(cL.get() + i * ic, ic, cH.get() + i * ic, wt.wave().lpr().data(), wt.wave().hpr().data(),
                              lf, xLp.get(), istride, ostride);
            ...
            if (iter == j - 1)
            {
                ...
                {
                    for (k = 0; k < wt.cols(); ++k) { oup[k + i * wt.cols()] = out[k + i * ic]; }
                }
            }
            else
            {
    ...
    for (iter = 0; iter < j; ++iter)
    {
        ...
        for (auto i = 0; i < ic; ++i)
        {
            idwtSymStride(orig + i, ir, wavecoeff + aLH + i, wt.wave().lpr().data(), wt.wave().hpr().data(), lf,
                          xLp.get(), istride, ostride);

            for (k = lf - 2; k < 2 * ir; ++k) { cL[(k - lf + 2) * ic + i] = xLp[k]; }

            idwtSymStride(wavecoeff + aHL + i, ir, wavecoeff + aHH + i, wt.wave().lpr().data(), wt.wave().hpr().data(),
                          lf, xLp.get(), istride, ostride);
        ...
        for (auto i = 0; i < ir; ++i)
        {
            idwtSymStride(cL.get() + i * ic, ic, cH.get() + i * ic, wt.wave().lpr().data(), wt.wave().hpr().data(), lf,
                          xLp.get(), istride, ostride);
...
auto swt2(WaveletTransform2D& wt, float* inp) -> std::unique_ptr<float[]>
{
    ...
    for (iter = 0; iter < j; ++iter)
    {
        ...
        for (auto i = 0; i < ir; ++i)
        {
            swtPerStride(m, orig + i * ic, ic, wt.wave().lpd().data(), wt.wave().hpd().data(), lp,
                         lpDn1.get() + i * colsI, colsI, hpDn1.get() + i * colsI, istride, ostride);
        }
        // Column Filtering and Row subsampling
        aHH                      = n - cdim;
        ...
        for (auto i = 0; i < ic; ++i)
        {
            swtPerStride(m, lpDn1.get() + i, ir, wt.wave().lpd().data(), wt.wave().hpd().data(), lp,
                         wavecoeff.get() + aLL + i, rowsI, wavecoeff.get() + aLH + i, istride, ostride);
        }

        for (auto i = 0; i < ic; ++i)
        {
            swtPerStride(m, hpDn1.get() + i, ir, wt.wave().lpd().data(), wt.wave().hpd().data(), lp,
                         wavecoeff.get() + aHL + i, rowsI, wavecoeff.get() + aHH + i, istride, ostride);
        }
...
auto iswt2(WaveletTransform2D& wt, float const* wavecoeffs, float* oup) -> void
{
    ...
    for (iter = j; iter > 0; iter--)
    {
        ...
        for (it2 = 0; it2 < m; ++it2)
        {
            ...
            shift = 0;
            idwtShift(shift, ir, ic, wt.wave().lpr().data(), wt.wave().hpr().data(), wt.wave().lpd().size(), a.get(),
                      h.get(), v.get(), d.get(), oup1.get());
            // oup2
            ir  = 0;
            ic  = 0;
            it3 = 0;
...
auto modwt(WaveletTransform2D& wt, float* inp) -> std::unique_ptr<float[]>
{
    ...
    for (iter = 0; iter < j; ++iter)
    {
        ...
        for (auto i = 0; i < ir; ++i)
        {
            modwtPerStride(m, orig + i * ic, ic, filt.get(), lp, lpDn1.get() + i * colsI, colsI,
                           hpDn1.get() + i * colsI, istride, ostride);
        }
        // Column Filtering and Row subsampling
        aHH                      = n - cdim;
        wt.coeffaccess[clen]     = aHH;
        ...
        for (auto i = 0; i < ic; ++i)
        {
            modwtPerStride(m, lpDn1.get() + i, ir, filt.get(), lp, wavecoeff.get() + aLL + i, rowsI,
                           wavecoeff.get() + aLH + i, istride, ostride);
        }

        for (auto i = 0; i < ic; ++i)
        {
            modwtPerStride(m, hpDn1.get() + i, ir, filt.get(), lp, wavecoeff.get() + aHL + i, rowsI,
                           wavecoeff.get() + aHH + i, istride, ostride);
        }
...
auto imodwt(WaveletTransform2D& wt, float* wavecoeff, float* oup) -> void
{
    ...
    for (iter = 0; iter < j; ++iter)
    {
        ...
        for (auto i = 0; i < ic; ++i)
        {
            ...
            imodwtPerStride(m, wavecoeff + aHL + i, ir, wavecoeff + aHH + i, filt.get(), lf, cH.get() + i, istride,
                            ostride);
        }
        ...
        for (auto i = 0; i < ir; ++i)
        {
            imodwtPerStride(m, cL.get() + i * ic, ic, cH.get() + i * ic, filt.get(), lf, oup + i * ic, istride,
                            ostride);
        }
...
auto getWT2Coeffs(WaveletTransform2D& wt, float* wcoeffs, int level, char const* type, int* rows, int* cols) -> float*
{
    ...
    {
        fmt::printf(
            "Error : The data is decomposed into %d levels so the acceptable values of level are between 1 and %d", j,
            j);
        exit(-1);
    }
