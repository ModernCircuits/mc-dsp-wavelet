...
{
    auto const pi = static_cast<T>(numbers::pi);
    return cos(static_cast<T>(order * i) * pi / static_cast<T>(size - 1));
}
    ...
    switch (type) {
        ...
        case WindowFunction::triangular: {
            ...
            for (std::size_t i = 0; i < size; ++i) {
                first[i] = static_cast<T>(1.0)
                         - std::abs((static_cast<T>(i) - halfSlots) / halfSlots);
            }
            break;
        }
        ...
        case WindowFunction::blackman: {
            ...
            for (std::size_t i = 0; i < size; ++i) {
                ...
                first[i]
                    = static_cast<T>(0.5 * (1 - alpha) - 0.5 * cos2 + 0.5 * alpha * cos4);
            }
            break;
        }
