...
auto RFFT::performRealToComplex(float const* inp, Complex<float>* oup) -> void
{
    ...
    for (auto i = 1; i < n2; ++i)
    {
        ...
        oup[i].real((coup[i].real() + coup[n2 - i].real() + (temp1 * data_[i].real()) + (temp2 * data_[i].imag()))
                    / 2.0F);
        oup[i].imag((coup[i].imag() - coup[n2 - i].imag() + (temp2 * data_[i].real()) - (temp1 * data_[i].imag()))
                    / 2.0F);
    }
